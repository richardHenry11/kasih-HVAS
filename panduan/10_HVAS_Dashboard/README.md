...
SYSTEM PAGE FIX (v4)
- Date & Time popup uses a child numeric keyboard inside the dialog.
- WiFi popup uses a child keyboard and switches the active textarea when SSID/password is tapped.
- System popups are sized to stay within the 800x480 display.
- Reboot and Shutdown controls are included; Shutdown is informational because software power-off is not supported by the current hardware.
